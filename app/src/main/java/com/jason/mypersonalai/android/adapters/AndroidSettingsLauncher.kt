...
class AndroidSettingsLauncherImpl(
    ...
    private fun destinationCandidates(request: String): List<Pair<String, String>> = when {
        ...
        request.contains("bluetooth") -> listOf(Settings.ACTION_BLUETOOTH_SETTINGS to "Bluetooth settings")
        ...
        request.contains("battery") || request.contains("power") -> listOf(Settings.ACTION_BATTERY_SAVER_SETTINGS to "battery settings")
        request.contains("display") || request.contains("screen") -> listOf(Settings.ACTION_DISPLAY_SETTINGS to "display settings")
        request.contains("sound") || request.contains("audio") || request.contains("volume") -> listOf(Settings.ACTION_SOUND_SETTINGS to "sound settings")
