...
class IntentResolverTest {
    ...
    fun navigationSynonymsOpenSettings() {
        ...
        requests.forEach { request ->
            assertEquals(IntentResolver.Intent.OPEN_SETTINGS, IntentResolver.resolve(request).intent)
        }
    }
    ...
    fun arbitraryWordsDoNotFallBackToDeviceInfo() {
        ...
        requests.forEach { request ->
            assertEquals(IntentResolver.Intent.UNKNOWN, IntentResolver.resolve(request).intent)
        }
    }

    @Test
    fun bareDeviceTargetsRemainDeviceInfo() {
    ...
    fun informationRequestsRemainDeviceInfo() {
        ...
        requests.forEach { request ->
            assertEquals(IntentResolver.Intent.DEVICE_INFO, IntentResolver.resolve(request).intent)
        }
    }

    @Test
    fun generalSettingsIsNotConfusedWithNotificationSettings() {
        assertEquals(IntentResolver.Intent.OPEN_SETTINGS, IntentResolver.resolve("open settings").intent)
        assertEquals("settings", IntentResolver.resolve("open settings").target)
        assertEquals(IntentResolver.Intent.OPEN_SETTINGS, IntentResolver.resolve("open notification settings").intent)
        assertEquals("notifications", IntentResolver.resolve("open notification settings").target)
    }
}
